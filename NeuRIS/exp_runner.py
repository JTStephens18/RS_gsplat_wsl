...
class Runner:
    ...
    def get_near_far(self, rays_o, rays_d, idx_img, depths, gs_render_conf  = None, image_perm = None, iter_i= None, pixels_x = None, pixels_y= None):
        log_vox = {}
        log_vox.clear()
        batch_size = len(rays_o)
    ...
    def train_nerf(self):
        ...
        for iter_i in tqdm(range(res_step)):
            ...
            if self.dataset_type == 'dtu':
                ...
                near, far = torch.zeros(len(rays_o), 1).to(self.device), self.sample_range_indoor * torch.ones(len(rays_o), 1).to(self.device)
            else:
                NotImplementedError
    ...
    def get_image_perm(self):
        if not self.nvs:
            ...
            lis = torch.tensor(lis, dtype=torch.long).to(self.device)
    ...
    def validate_image(self, idx=-1, resolution_level=-1, 
        ...
        idx_pixels_vu = torch.tensor([[i, j] for i in range(0, H) for j in range(0, W)]).to(self.device).split(self.batch_size)
    ...
    def validate_fields(self, iter_step=-1):
        if iter_step < 0:
            iter_step = self.iter_step
        bound_min = torch.tensor(self.dataset.bbox_min, dtype=torch.float32).to(self.device)
        bound_max = torch.tensor(self.dataset.bbox_max, dtype=torch.float32).to(self.device)
    ...
    def validate_image_nerf(self, idx=-1, resolution_level=-1, save_render = False):
        ...
        for rays_o_batch, rays_d_batch in zip(rays_o, rays_d):
            near, far = torch.zeros(len(rays_o_batch), 1).to(self.device), 1 * torch.ones(len(rays_o_batch), 1).to(self.device)
            if self.dataset_type == 'dtu':
