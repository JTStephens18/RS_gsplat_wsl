...
def prepare_patches_src(img_gray, indices_pixel, window_size=11, window_step=2):
    ...
    window_size+=1
    x = torch.arange(-(window_size//2), window_size//2 + 1, window_step).to("cuda" if torch.cuda.is_available() else "cpu") # weired -1//2=-1
...
def convert_to_homo(pts):
    pts_homo = torch.cat([pts, torch.ones(pts.shape[:-1] + tuple([1])).to("cuda" if torch.cuda.is_available() else "cpu") ], dim=-1)
    return pts_homo
