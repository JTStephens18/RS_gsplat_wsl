...
class FixVarianceNetwork(nn.Module):
    ...
    def forward(self, x):
        return torch.ones([len(x), 1]).to(self.device) * np.exp(-self.iter_step / self.base)
...
class SingleVarianceNetwork(nn.Module):
    def __init__(self, init_val=1.0, use_fixed_variance = False):
        ...
        if use_fixed_variance:
            ...
            self.variance = torch.tensor([init_val]).to("cuda" if torch.cuda.is_available() else "cpu")
        else:
            self.register_parameter('variance', nn.Parameter(torch.tensor(init_val).to("cuda" if torch.cuda.is_available() else "cpu")))

    def forward(self, x):
        return torch.ones([len(x), 1]).to("cuda" if torch.cuda.is_available() else "cpu") * torch.exp(self.variance * 10.0)
