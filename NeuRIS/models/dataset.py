...
class Dataset:
    ...
    def __init__(self, conf, args):
        ...
        if self.estimate_scale_mat:
            ...
            self.scale_mats_np = np.array([np.eye(4) for _ in range(len(self.world_mats_np))]) * self.conf["sdf_scene_scale"]
    ...
    def gen_rays(self):
        tx = torch.linspace(0, self.W - 1, self.W).to(self.device)
        ty = torch.linspace(0, self.H - 1, self.H).to(self.device)
    ...
    def gen_random_rays_at(self, img_idx, batch_size):
        ...
        return rays_o, rays_v, color, mask
    ...
    def score_pixels_ncc(self, idx, pts_world, normals_world, pixels_coords_vu, reso_level = 1.0, _debug = False):
        ...
        scores_all_mean, diff_patch_all, count_valid_all = torch.zeros(num_patches, dtype=torch.float32).to(self.device), torch.zeros(num_patches, dtype=torch.float32).to(self.device), torch.zeros(num_patches, dtype=torch.uint8).to(self.device)
