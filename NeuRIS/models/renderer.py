...
def sample_pdf(bins, weights, n_samples, det=False):
    ...
    if det:
        u = torch.linspace(0. + 0.5 / n_samples, 1. - 0.5 / n_samples, steps=n_samples).to("cuda" if torch.cuda.is_available() else "cpu")
        u = u.expand(list(cdf.shape[:-1]) + [n_samples])
    else:
        u = torch.rand(list(cdf.shape[:-1]) + [n_samples]).to("cuda" if torch.cuda.is_available() else "cpu")
...
class NeuSRenderer:
    ...
    def render_core_outside(self, rays_o, rays_d, z_vals, sample_dist, nerf, background_rgb=None):
        ...
        dists = torch.cat([dists, torch.Tensor([sample_dist]).to(self.device).expand(dists[..., :1].shape)], -1)

        mid_z_vals = z_vals + dists * 0.5
        mid_dists = mid_z_vals[..., 1:] - mid_z_vals[..., :-1]
        mid_dists = torch.cat([mid_dists, torch.Tensor([sample_dist]).to(self.device).expand(dists[..., :1].shape)], -1)
        ...
        weights = alpha * torch.cumprod(torch.cat([torch.ones([batch_size, 1]).to(self.device), 1. - alpha + 1e-7], -1), -1)[:, :-1]  # n_rays, n_samples
    ...
    def up_sample(self, rays_o, rays_d, z_vals, sdf, n_importance, inv_variance):
        ...
        if self.alpha_type == 'uniform':
            dot_val = torch.ones([batch_size, n_samples - 1]).to(self.device) * -1.0
        else:
            dot_val = (next_sdf - prev_sdf) / (next_z_vals - prev_z_vals + 1e-5)
            prev_dot_val = torch.cat([torch.zeros([batch_size, 1]).to(self.device), dot_val[:, :-1]], dim=-1)
        ...
        weights = alpha * torch.cumprod(
            torch.cat([torch.ones([batch_size, 1]).to(self.device), 1. - alpha + 1e-7], -1), -1)[:, :-1]
    ...
    def render_core(self,
        ...
        inv_variance = variance_network(torch.zeros([1, 3]).to(self.device))[:, :1].clip(1e-6, 1e6)
        ...
        with torch.no_grad():
            if inv_variance[0, 0] > 800:
                ...
                inv_variance2 = torch.tensor([800]).to(self.device)
                ...
                weights2 = alpha2 * torch.cumprod(torch.cat([torch.ones([batch_size, 1]).to(self.device), 1. - alpha2 + 1e-7], -1), -1)[:, :-1]  # n_rays, n_samples
    ...
    def render(self, rays_o, rays_d, near, far, perturb_overwrite=-1, background_rgb=None, alpha_inter_ratio=0.0):
        ...
        if self.n_importance > 0:
            with torch.no_grad():
                ...
                sdf = self.sdf_network_fine.sdf(pts.reshape(-1, 3)).reshape(batch_size, self.n_samples).to(self.device)
